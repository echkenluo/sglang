...
"""F28 indexer kernels, ported from vLLM commit 0d01ff31997b.

Copied unchanged from our DSV4 SM89 branch (integration-v0517,
python/sglang/srt/layers/attention/dsv4/f28_mqa.py at e61eac7b5db7) for the
GLM-5.3 k-pool indexer on SM89, where DeepGEMM has no kernels.

The packed page stores all FP8 keys followed by all FP32 scales. Preserve
64-bit physical-page arithmetic and rowwise tensor-core dispatch.
"""
...
def fp8_mqa_logits_triton(
    ...
    *,
    ...
    if native_fp8 is None:
        native_fp8 = torch.cuda.get_device_capability(q.device)[0] == 12
...
def sglang_paged_mqa_logits(
    ...
    if clean_logits:
        raise ValueError("F28 adapter expects clean_logits=False")
