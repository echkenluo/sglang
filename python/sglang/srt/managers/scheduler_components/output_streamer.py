...
class _GenerationStreamAccumulator:
    ...
    def accept(self, *, req: Req) -> None:
        ...
        # Emit close-kind exactly on the accepted </think> token.  Keeping this
        # token-aligned lets the existing customized_info streaming machinery
        # handle normal, streaming, and speculative output without a new IPC
        # shape.  The grammar state persists after the close, but already-sent
        # tokens are excluded by send_token_offset, so the marker is emitted
        # once.
