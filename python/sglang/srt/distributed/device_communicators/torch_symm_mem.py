...
class TorchSymmMemCommunicator:
    ...
    def __init__(
        ...
        *,
        ...
        if self.fused_only:
            # The DeepSeek-V4 fused AG/RS paths allocate their exact-shape
            # symmetric buffers lazily.  Do not reserve the generic
            # AllReduce buffer: it is never read by those paths and reduces
            # the KV pool before the first request.
            self.disabled = False
            return
    ...
    def set_use_cp(self, value: bool) -> None:
        """Set the per-layer CP fused-kernel scope."""
        if value and self.use_cp:
            raise RuntimeError("nested torch symmetric-memory CP scope")
        self.use_cp = value

    @contextmanager
    def cp_fused_scope(self):
        """Activate fused CP wrappers for one MoE layer and always reset."""
        self.set_use_cp(True)
        try:
            yield
        finally:
            self.set_use_cp(False)
    ...
    def get_or_create_ag_gemm_ctx(
        ...
        self._ag_gemm_ctx = create_allgather_gemm_context_symm_mem(
            ...
            rank=dist.get_rank(self.group),
            world_size=self.world_size,
            max_M=(max_M + self.world_size - 1) // self.world_size,
            K=K,
