...
def maybe_fused_ag_shared_experts(
    ...
    if not shared_experts_is_fp8:
        raise RuntimeError("fused CP AG+GEMM requires FP8 shared experts")
    if w_fp8 is None or w_scale is None:
        raise RuntimeError("fused CP AG+GEMM requires FP8 weight and scale")
    if hidden_states.ndim != 2 or hidden_states.dtype != torch.bfloat16:
        raise RuntimeError(
            "fused CP AG+GEMM requires a 2D BF16 activation, got "
            f"shape={tuple(hidden_states.shape)} dtype={hidden_states.dtype}"
        )
    if not hidden_states.is_contiguous():
        raise RuntimeError("fused CP AG+GEMM requires contiguous activation")
    _, K = hidden_states.shape

    ctx = comm.get_or_create_ag_gemm_ctx(K=K)
    if ctx is None:
        raise RuntimeError("failed to create fused CP AG+GEMM context")
