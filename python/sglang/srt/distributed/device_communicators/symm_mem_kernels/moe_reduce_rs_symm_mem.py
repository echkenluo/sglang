...
def _cas_sys_release(addrs, expected, desired):
    ...
    return tl.inline_asm_elementwise(
        """
        {
            ...
            .reg .pred  %p<1>;
            cas_loop:
                atom.global.release.sys.cas.b32 %tmp32_0, [$1], $2, $3;
                setp.eq.u32 %p0, %tmp32_0, $2;
                @!%p0 bra cas_loop;
            mov.u32 $0, %tmp32_0;
        }
        """,
        "=r, l, r, r",
        [addrs, expected, desired],
        dtype=tl.uint32,
        is_pure=False,
        pack=1,
    )
...
def _cas_sys_acquire(addrs, expected, desired):
    ...
    return tl.inline_asm_elementwise(
        """
        {
            ...
            .reg .pred  %p<1>;
            cas_loop:
                atom.global.acquire.sys.cas.b32 %tmp32_0, [$1], $2, $3;
                setp.eq.u32 %p0, %tmp32_0, $2;
                @!%p0 bra cas_loop;
            mov.u32 $0, %tmp32_0;
        }
...
def maybe_fused_shared_add_rs(
    ...
    if shared_output is None:
        raise RuntimeError("fused CP MoE-RS requires shared-expert output")
    if final_hidden_states.ndim != 3:
        raise RuntimeError(
            "fused CP MoE-RS requires unreduced [M, topk, N] output, got "
            f"shape={tuple(final_hidden_states.shape)}"
        )
    M, _, N = final_hidden_states.shape
    if M == 0 or (M % tp_size) != 0:
        raise RuntimeError(
            f"fused CP MoE-RS requires M divisible by TP size, got M={M}, tp={tp_size}"
        )
    if final_hidden_states.shape[1] != top_k:
        raise RuntimeError(
            "fused CP MoE-RS top-k mismatch: "
            f"tensor={final_hidden_states.shape[1]} configured={top_k}"
        )
    if shared_output.shape != (M, N):
        raise RuntimeError(
            "fused CP MoE-RS shared output shape mismatch: "
            f"expected={(M, N)} got={tuple(shared_output.shape)}"
        )
    if N % 512 != 0:
        raise RuntimeError(f"fused CP MoE-RS requires N divisible by 512, got N={N}")
    ...
    if ctx is None:
        raise RuntimeError("failed to create fused CP MoE-RS context")
