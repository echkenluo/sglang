...
def test_nonstream_multichoice_shares_prompt_but_sums_spec():
    ...
    responses = [
        {
            "meta_info": _meta(completion=60, verify=12, correct=30, proposed=180)
            | {"prompt_tokens": 1000}
        },
        {
            "meta_info": _meta(completion=40, verify=12, correct=10, proposed=180)
            | {"prompt_tokens": 1000}
        },
    ]
...
def test_streaming_last_chunk_cumulative_semantics():
    ...
    assert (
        stream_usage.sglang_spec_details.accept_length
