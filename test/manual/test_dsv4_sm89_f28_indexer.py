...
class TestF28Indexer(unittest.TestCase):
    def test_physical_page_address_exceeds_int32(self):
        page, dim, heads = 64, 128, 64
        stride = page * (dim + 4)
        physical = (2**31 // stride) + 2
        cache = torch.empty(
            physical + 1, page, 1, dim + 4, device="cuda", dtype=torch.uint8
        )
        keys = torch.ones(page, dim, device="cuda").to(torch.float8_e4m3fn)
        scales = torch.full((page,), 0.125, device="cuda")
        cache[physical].reshape(-1).copy_(
            torch.cat((keys.view(torch.uint8).flatten(), scales.view(torch.uint8)))
        )
        q = torch.ones(1, 1, heads, dim, device="cuda").to(torch.float8_e4m3fn)
        weights = torch.ones(1, heads, device="cuda")
        lengths = torch.tensor([[63]], device="cuda", dtype=torch.int32)
        tables = torch.tensor([[physical]], device="cuda", dtype=torch.int32)
        out = sglang_paged_mqa_logits(
            q, cache, weights, lengths, tables, None, 64, False
        )
        torch.testing.assert_close(
            out[:, :63], torch.full((1, 63), 1024.0, device="cuda"), atol=0, rtol=0
        )
        self.assertTrue(torch.isneginf(out[0, 63]).item())
        print(
            f"INDEXER physical_page={physical} byte_offset={physical*stride} passed",
            flush=True,
        )
    ...
    def test_paged_logits_and_graph_updates(self):
        ...
        keys = (torch.randn(pages, page, d, device="cuda") * 0.5).to(
            torch.float8_e4m3fn
        )
        scales = torch.rand(pages, page, device="cuda") * 0.02 + 0.01
        packed = torch.cat(
            (
                keys.view(torch.uint8).reshape(pages, -1),
        ...
        storage = torch.zeros(
            packed.numel() + page * 132, device="cuda", dtype=torch.uint8
        )
        ...
        for rows in (1, 6, 16, 80, 513):
            ...
            def call():
                return sglang_paged_mqa_logits(
                    q, cache, weights, lengths, tables, None, 317, False
                )
            ...
            def check(out):
                ...
                torch.testing.assert_close(
                    out[valid], ref[valid], atol=0.002, rtol=0.002
                )
            ...
            for update in range(3):
                ...
                tables.copy_(
                    torch.randint(pages, tables.shape, device="cuda", dtype=torch.int32)
                )
                lengths.copy_(
                    torch.randint(
                        0, 318, lengths.shape, device="cuda", dtype=torch.int32
                    )
                )
                graph.replay()
                check(captured)
