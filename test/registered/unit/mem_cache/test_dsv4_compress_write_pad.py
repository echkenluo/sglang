...
class TestCompressStateWritePad(CustomTestCase):
    ...
    @staticmethod
    def _make_c4_configurator() -> DSV4PoolConfigurator:
        configurator = object.__new__(DSV4PoolConfigurator)
        configurator.c4_ring_size = get_compress_state_ring_size(4, True)
        configurator.c128_ring_size = get_compress_state_ring_size(128, True)
        configurator.num_layers_ca4 = 1
        configurator.num_layers_ca128 = 0
        return configurator

    def test_capacity_gate_accepts_ring_limit(self):
        configurator = self._make_c4_configurator()
        configurator._assert_ring_serves_draft_tokens(10)

    def test_capacity_gate_rejects_overflow(self):
        configurator = self._make_c4_configurator()
        with self.assertRaisesRegex(
            AssertionError,
            r"speculative_num_draft_tokens=11.*c4.*serves at most 10",
        ):
